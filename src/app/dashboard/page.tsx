...
async function getQuotes(
  ...
  if (search) {
    query = query.or(
      `customer_company.ilike.%${search}%,customer.full_name.ilike.%${search}%`,
    );
  }
...
async function getStats(useServiceRole = false) {
  const supabase = useServiceRole
    ? createServiceClient()
    : await createClient();
...
export default async function DashboardPage({
  ...
  const [quotes, stats, reps] = await Promise.all([
    getQuotes(params.search, params.status, params.rep, isAdmin),
    getStats(isAdmin),
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">
          Manage and track all incoming quote requests
        </p>
      </div>
